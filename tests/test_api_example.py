...
6. Get car details based on car_type GET with URL params
7. Register car - POST request(with url_params)
8. Get list of registered cars -GET
9. Verify registered cars count
10. Delete newly added car -DELETE request
"""
import os,sys,time
...
def test_api_example(api_url='http://35.167.62.251/'):
    ...
    try:
        # Create test object
        test_obj = API_Player(user_account,url=api_url)
        expected_pass = 0
        actual_pass = -1
        ...
        # Get one car based on type
        car_type   =  conf.car_type
        result_flag=test_obj.filter_cars(auth_details=auth_details,
                                         car_type=car_type)

        test_obj.log_result(result_flag,
                            positive='Succefully added: %s' % car_type,
                            negative='Not added successfully: %s'% car_type)
